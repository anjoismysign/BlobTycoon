...
public class RackAssetHeldTutorialListener extends BlobTycoonListener {

    public RackAssetHeldTutorialListener(TycoonListenerManager listenerManager) {
    ...
    public void onHeld(PlayerItemHeldEvent event) {
        ...
        map.forEach((primitiveAsset, tycoonModelHolder) -> {
            if (primitiveAsset != PrimitiveAsset.RACK)
                return;
            BlobLibMessageAPI.getInstance()
                    .getMessage("BlobTycoon.RackAsset-Held-Tutorial", player)
                    .handle(player);
        });
    }

    public boolean checkIfShouldRegister() {
        return getConfigManager().getRackAssetHeldTutorial().register();
    }
}
